...
private:
    ...
    struct DurationConfig {
        ...
        nsecs_t appEarly = -1;
        nsecs_t sfEarlyGl = -1;
        nsecs_t appEarlyGl = -1;
        nsecs_t sfLate = -1;
        nsecs_t appLate = -1;
    };

    const DurationConfig* durationConfigForHz(int hz) const;

    static DurationConfig loadDurationConfig(int hz);
    static nsecs_t durationForPrefix(const DurationConfig& config, const char* prefix);
    ...
    DurationConfig mDuration60;
    DurationConfig mDuration90;
    DurationConfig mDuration120;
