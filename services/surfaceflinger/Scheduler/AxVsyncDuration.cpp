...
constexpr const char* kPrefixSfEarly = "sf.early.";
constexpr const char* kPrefixAppEarly = "app.early.";
constexpr const char* kPrefixSfEarlyGl = "sf.earlyGl.";
constexpr const char* kPrefixAppEarlyGl = "app.earlyGl.";
constexpr const char* kPrefixSfLate = "sf.late.";
constexpr const char* kPrefixAppLate = "app.late.";
...
AxVsyncDuration::AxVsyncDuration()
    ...
    if (mDynamicSwitchEnabled) {
        ...
        mDuration60 = loadDurationConfig(60);
        mDuration90 = loadDurationConfig(90);
        mDuration120 = loadDurationConfig(120);
...
AxVsyncDuration::DurationConfig AxVsyncDuration::loadDurationConfig(int hz) {
    const auto load = [hz](const char* prefix) {
        const std::string key = base::StringPrintf("%s%s%d", kPropBasePath, prefix, hz);
        return loadAxVsyncDurationProperty(key.c_str());
    };

    DurationConfig config;
    config.sfEarly = load(kPrefixSfEarly);
    config.appEarly = load(kPrefixAppEarly);
    config.sfEarlyGl = load(kPrefixSfEarlyGl);
    config.appEarlyGl = load(kPrefixAppEarlyGl);
    config.sfLate = load(kPrefixSfLate);
    config.appLate = load(kPrefixAppLate);
    return config;
}

const AxVsyncDuration::DurationConfig* AxVsyncDuration::durationConfigForHz(int hz) const {
    switch (hz) {
        case 60:
            return &mDuration60;
        case 90:
...
nsecs_t AxVsyncDuration::durationForPrefix(const DurationConfig& config, const char* prefix) {
    if (std::strcmp(prefix, kPrefixSfEarly) == 0) return config.sfEarly;
    if (std::strcmp(prefix, kPrefixAppEarly) == 0) return config.appEarly;
    if (std::strcmp(prefix, kPrefixSfEarlyGl) == 0) return config.sfEarlyGl;
    if (std::strcmp(prefix, kPrefixAppEarlyGl) == 0) return config.appEarlyGl;
    if (std::strcmp(prefix, kPrefixSfLate) == 0) return config.sfLate;
    if (std::strcmp(prefix, kPrefixAppLate) == 0) return config.appLate;
    return -1;
}
...
nsecs_t AxVsyncDuration::loadPerHzDuration(const char* prefix, nsecs_t vsyncPeriod,
    ...
    const DurationConfig* config = durationConfigForHz(hz);
    if (config) {
        const nsecs_t val = durationForPrefix(*config, prefix);
        if (val > 0) return val;
    }
